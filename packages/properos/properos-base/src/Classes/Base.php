...
abstract class Base extends Paginator
{
    ...
    public function deleteModel($id, $where = [])
    {
        ...
        if (is_array($id) && count($id) > 0) {
            return Api::success(\str_plural($this->title) . ' were successfully deleted.', $this->model->whereIn('id', $id)->delete());
        } elseif ($id > 0) {
            ...
            switch (strtolower($key)) {
                case 'query':
                    if (is_string($value)) {
                        ...
                        $options['q'] = array_get($value, 'value', '');
                        $options['q_fields'] = array_get($value, 'fields', []);
                    }
                    break;
                case 'fields_raw':
                case 'where_raw':
    ...
    public function normalize_query($query)
    {
        ...
        if (is_string($query) && $query != '') {
            ...
            foreach ($aQuery as $value) {
                if (starts_with($value, '+*') || (starts_with($value, '+') && ends_with($value, '*'))) {
        ...
        if ($collection) {
            ...
            if (isset($options['fields']) && count($options['fields']) > 0) {
                ...
                $res = $collections->map(function ($item) use ($options) {
                    return array_only($item->toArray(), $options['fields']);
                });
                return $res->all();
            } else {
